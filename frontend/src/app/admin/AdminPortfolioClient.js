...
function itemToForm(item) {
  return {
    ...
    category: item.category || "AI Products",
    image: item.image || "/banner-aboutus.webp",
    route: item.route || "/portfolio",
    tags: (item.tags || []).join(", "),
...
export default function AdminPortfolioClient() {
  const [items, setItems] = useState(portfolioSeed);
  ...
  useEffect(() => {
    ...
    async function loadItems() {
      try {
        ...
        if (!active) return;
        setItems(Array.isArray(data.items) ? data.items : portfolioSeed);
        setMessage("Portfolio loaded");
      } catch (error) {
        if (!active) return;
        setMessage(error?.message || "Cannot load portfolio");
      }
    }
  ...
  function handleNew() {
    setEditingSlug("");
    setForm(emptyForm());
    setMessage("Creating new portfolio item");
  }
  ...
  async function handleSave() {
    ...
    try {
      const endpoint = editingSlug
        ? `/api/admin/portfolio/${editingSlug}`
        : "/api/admin/portfolio";
  ...
  async function handleDelete(slug) {
    try {
      const res = await fetch(apiUrl(`/api/admin/portfolio/${slug}`), {
        method: "DELETE",
        credentials: "include",
      });
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <section className="mb-6 rounded-[30px] border border-white/80 bg-white/90 p-6 shadow-[0_18px_55px_rgba(15,23,42,0.08)]">
          ...
          <p className="mt-1 text-sm text-slate-600">
            Create, edit, and delete portfolio cards shown in the main portfolio page.
          </p>
        </section>
        ...
        <div className="grid gap-5 lg:grid-cols-[0.95fr_1.05fr]">
          <section className="rounded-[28px] border border-slate-200 bg-white p-4 shadow-[0_14px_40px_rgba(15,23,42,0.06)] sm:p-5">
            <div className="flex items-center justify-between gap-2">
              <h2 className="text-lg font-semibold text-slate-900">Portfolio Items</h2>
              <button
                type="button"
                onClick={handleNew}
            ...
            <div className="mt-3 space-y-2">
              ...
              {paginatedItems.length === 0 ? (
                <p className="rounded-xl border border-dashed border-slate-200 bg-slate-50 p-3 text-sm text-slate-500">
                  No portfolio items found for your search.
                </p>
              ) : null}
            </div>
          ...
          <section className="rounded-[28px] border border-slate-200 bg-white p-4 shadow-[0_14px_40px_rgba(15,23,42,0.06)] sm:p-5">
            <h2 className="text-lg font-semibold text-slate-900">
              {editingSlug ? "Edit Portfolio Item" : "Create Portfolio Item"}
            </h2>
